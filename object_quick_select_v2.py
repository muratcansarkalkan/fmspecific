...
COL_SELECT_ALL  = (0.20, 0.75, 0.45, 1.0)   # green tint for "select all" flash
COL_BADGE_BG    = (0.30, 0.30, 0.30, 1.0)
COL_BADGE_TEXT  = (0.80, 0.80, 0.80, 1.0)
...
class OBJECT_OT_quick_select(bpy.types.Operator):
    ...
    _flash    = False     # briefly highlight input box green after select-all
    ...
    def _select_all_filtered(self, context, add=False):
        """Select every object currently visible in the filtered list."""
        if not self._items:
            return 0
        if not add:
            for o in context.selected_objects:
    ...
    def _draw(self, context):
        ...
        input_border = COL_SELECT_ALL if self._flash else COL_HIGHLIGHT
        draw_rect(x, y_input, BOX_WIDTH, INPUT_HEIGHT, COL_INPUT_BG)
        draw_rect_outline(x, y_input, BOX_WIDTH, INPUT_HEIGHT, input_border)
        ...
        # ── item count badge (top-right of input box) ──
        count_str = f"{len(self._items)} obj{'s' if len(self._items) != 1 else ''}"
        cw = text_width(count_str, size=13) + PADDING
        bx = x + BOX_WIDTH - cw - 4
        by = y_input + (INPUT_HEIGHT - 18) // 2
        draw_rect(bx, by, cw, 18, COL_BADGE_BG)
        draw_text(count_str, bx + PADDING // 2, by + 2, size=13, color=COL_BADGE_TEXT)

        # ── hint line at bottom ──
        hint_y = y_drop - 20
        draw_text(
            "Enter=select  Shift+Enter=add  Ctrl+Enter=select ALL  Ctrl+Shift+Enter=add ALL  Esc=cancel  ↑↓=navigate",
            x, hint_y, size=12, color=COL_DIM_TEXT)
    ...
    def modal(self, context, event):
        ...
        if event.type in ('RET', 'NUMPAD_ENTER') and event.value == 'PRESS':
            if event.ctrl:
                ...
                count = self._select_all_filtered(context, add=event.shift)
                self._flash = True
